...
namespace Apa.Repo {
    public async int remove (
        ...
        args_handler.init_options (
            OptionData.concat (AptRepo.Data.COMMON_OPTIONS_DATA, AptRepo.Data.RM_OPTIONS_DATA),
            OptionData.concat (AptRepo.Data.COMMON_ARG_OPTIONS_DATA, AptRepo.Data.RM_ARG_OPTIONS_DATA),
        ...
        while (true) {
            error.clear ();
            var status = yield AptRepo.rm (args_handler, error, skip_unknown_options);
